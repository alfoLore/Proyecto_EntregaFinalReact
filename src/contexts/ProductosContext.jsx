...
export function ProductosProvider({ children }) {
    ...
    const agregarProducto = (producto) => {
        return(
            new Promise(async (res, rej) => {
                try {
                    const respuesta = await fetch('https://687fa0a5efe65e52008a71ab.mockapi.io/productos', {
                        method: 'POST',
                        headers: {
    ...
    function editarProducto(producto){
        return(
            new Promise(async(res, rej) => {
            try {
                const respuesta = await fetch(`https://687fa0a5efe65e52008a71ab.mockapi.io/productos/${producto.id}`, {
                    method: 'PUT',
                    headers: {
                    'Content-Type': 'application/json',
                    },
    ...
    const eliminarProducto = (id) => {
        ...
        if (confirmar) {
            return(
                new Promise(async (res, rej) => {
                    try {
                        const respuesta = await fetch(`https://687fa0a5efe65e52008a71ab.mockapi.io/productos/${id}`, {
                        method: 'DELETE',
                        });
